...
def main():
    ...
    if not user:
        ...
        # --- Firestore/SQLite indicator on login page ---
        if 'USE_FIREBASE' in globals() and USE_FIREBASE and FIRESTORE_CLIENT is not None:
            st.success("🟢 Tersambung ke Firestore (Cloud Database)")
        else:
            st.warning("🟡 Tidak tersambung ke Firestore, menggunakan SQLite lokal.")
